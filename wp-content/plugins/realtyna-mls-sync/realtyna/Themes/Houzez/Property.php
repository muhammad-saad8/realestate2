...
class Property {
        ...
        if ( in_array( strtolower(  $urlExt  ), $allowedExtensions ) ){
            ...
            if ( !empty( $info ) && is_array( $info ) ) {
                ...
                if ( $attachmentId !== 0 || !is_wp_error( $attachmentId ) ) {
                    ...
                    $this->addMetaToAttachment($attachmentId);
			...
			if ( $fileStored ){
                ...
                if ( !is_wp_error( $attachId ) ) {
                    ...
                    if ( $generateThumbnails ) {
                        ...
                        $this->addMetaToAttachment($attachId);
    ...
    /**
     * Add common metadata to a WordPress attachment post.
     *
     ...
     */
    private function addMetaToAttachment($attachmentId)
    {

        if(!get_post($attachmentId) || get_post_type($attachmentId) != 'attachment') {
        ...
        $images_alternative_text = $this->getValue('images_alternative_text');
        if ( isset($images_alternative_text) ) {

            update_post_meta( $attachmentId, '_wp_attachment_image_alt', $images_alternative_text );

        }

        $images_title = $this->getValue('images_title');
        if ( isset($images_title) ) {

            $updated_post_data = array(
                'ID'         => $attachmentId,
                'post_title' => $images_title,
            );
        ...
        $images_caption = $this->getValue('images_caption');
        if ( isset($images_caption) ) {

            $updated_post_data = array(
                'ID'         => $attachmentId,
                'post_excerpt' => $images_caption,
            );
        ...
        $images_description = $this->getValue('images_description');
        if ( isset($images_description) ) {

            $updated_post_data = array(
                'ID'         => $attachmentId,
                'post_content' => $images_description,
            );
